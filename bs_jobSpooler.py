...
def setupSceneGlobals():
    try:
        prjRoot = os.getenv('SHOW')
        if not prjRoot:
            prjRoot = '/nas/projects/Tactic/bilal'
        renderRoot='%s/render'%prjRoot
        ...
        setAttr("defaultArnoldRenderOptions.abortOnError", 0)
        ...
        # needed by Massive Arnold Procedural
        setAttr("defaultArnoldRenderOptions.aiUserOptions", 'declare render_pass constant STRING render_pass "renderpass_Arnold" declare motion_blur constant INT motion_blur 1')
        ...
        geos = ls(type='surfaceShape')
        for geo in geos:
            displaySmoothness(geo, divisionsU=0, divisionsV=0, pointsWire=4, pointsShaded=1, polygonObject=1)
...
class bs_jobSpooler(QtGui.QDialog):
    def __init__(self, parent=mayaMainWindow(), imgPath=None, tskType=0, tskFile=False, prjPath=False, tskScript='', opt='',
        ...
        self.mayaLoc = os.getenv('MAYA_LOCATION')
        if not self.showName:
            self.showName = 'bilal'
        if not self.mayaLoc:
            self.mayaLoc = '/usr/autodesk/maya2014-x64'
    ...
    def genTask(self, internal=True):
        ...
        for s in jobRange:
            ...
            if self.taskIndex == 0:
                ...
                jobTitle = '{' + str(self.ui.taskTitle.text()) + '} '
                taskTitle = '{Renders ' + str(jobStart) + '-' + str(jobEnd) + '} '
                taskCmd = '%s/bin/Render '%self.mayaLoc
                taskArgs = ''
                service = '"PixarRender%s" %s' %(extraService, slotOpt)
                tag = '{prman}'
                if taskBaseFilename[-1:] == '.':
                ...
                taskTitle = '{Renders ' + str(jobStart) + '-' + str(jobEnd) + '} '
                extraArgs = self.extraArgs
                taskCmd = '%s/bin/maya -batch '%self.mayaLoc
                taskArgs = ''
                service = '"PixarRender%s" %s' %(extraService, slotOpt)
                tag = '{prman}'
                if taskBaseFilename[-1:] == '.':
                ...
                jobTitle = '{' + str(self.ui.taskTitle.text()) + '} '
                taskTitle = '{Renders ' + str(jobStart) + '-' + str(jobEnd) + '} '
                taskCmd = '%s/bin/maya -batch -command '%self.mayaLoc
                taskArgs = ''
                service = '"PixarRender%s" %s' %(extraService, slotOpt)
                tag = '{prman}'
                if taskBaseFilename[-1:] == '.':
        ...
        if 'fox' in self.engine:
            if self.taskIndex == 5 or self.taskIndex == 4:
                ...
                #####
                scn_elements = os.path.basename(sceneName()).split(".")[0].split("_")[0:-1]
                render_path = pm.general.getAttr("defaultRenderGlobals.imageFilePrefix")
                #scn_elements.append(re.search("v\d{3}", render_path).group())
                prePubInfo = runCommands.parseSelectedFiles(sceneName())
                scn_elements.append(prePubInfo["version"])
                
                task_title = "_".join(scn_elements)
                ...
                self.task_uuid = "{0}__{1}".format(task_title, self.uuid)
    ...
    def sendToTractor(self, internal=True):
        ...
        if self.taskIndex == 5 or self.taskIndex == 4:
            ...
            # CTRL+'sendToTractor': skip scene publishing
        ...
        if internal:
            ...
            if not self.err:
                ...
                if self.taskIndex == 5 and not published:
                    return
                else:
                    self.genMsgEnabled = True
                    Dismiss = 'Dismiss'
    ...
    def submit_to_remote_tractor(self):
        ...
        if launch_type is not None:
            cmd_str = "{0} {1} {2} {3} {4} {5} {6}".format(Q_SUBMIT_BIN, launch_type, file_path, getpass.getuser(), self.jobFullPath, self.uuid, self.priority) 
            ...
            /nas/projects/Tactic/sandbox/bilal/abhishek/sequences/seq13/scn27/sh006_SHOT00000128/lighting/lighting/scenes//seq13_scn27_sh006_lig_lighting_v008.alf caf1f7d1-2a42-4810-a171-2973c82de9ed 80
            """
            # Subprocess to launch the Q_SUBMIT_BIN
            import subprocess
            p = subprocess.Popen([Q_SUBMIT_BIN, launch_type, '{0}'.format(file_path), '{0}'.format(getpass.getuser()), '{0}'.format(self.jobFullPath), '{0}'.format(self.uuid), '{0}'.format(self.priority),], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
