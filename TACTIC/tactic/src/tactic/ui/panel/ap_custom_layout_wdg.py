...
#
#
__all__ = ["AP_CustomLayoutWdg", "SObjectHeaderWdg"]
...
class AP_CustomLayoutWdg(BaseRefreshWdg):
    ...
    def __init__(my, **kwargs):
        super(AP_CustomLayoutWdg, my).__init__(**kwargs)
    ...
    def get_display(my):
        ...
        top.add_class("spt_custom_abhishek")
    ...
    def process_mako(my, html):
        ...
        html = '%s%s' % (AP_CustomLayoutWdg.HEADER, html)
    ...
    def get_config(my):
        ...
        # this is the new preferred way of defining AP_CustomLayoutWdg
        search = Search("config/widget_config")
        if my.category:
            search.add_filter("category", my.category)
        else:
            search.add_filter("category", 'AP_CustomLayoutWdg')
        if my.search_type:
        ...
        # deprecated approach, assuming a "AP_CustomLayoutWdg" as search_type,
        # is deprecated
        if not config:
            search = Search("config/widget_config")
            if my.category:
                search.add_filter("category", my.category)
            if my.search_type:
                search.add_filter("search_type", "AP_CustomLayoutWdg")
    ...
    def get_def_config(my, def_xml=None):
        ...
        if my.def_view:
            ...
            my.search_type = "AP_CustomLayoutWdg"
            search = Search("config/widget_config")
            search.add_filter("search_type", my.search_type)
    ...
    # FIXME: this is all necessary because AP_CustomLayoutWdg is not derived from
    # BaseTableElementWdg ...  AP_CustomLayoutWdg should probably not be used
    # as a table elementj
    ...
    def get_element_wdg(my, xml, def_config):
        ...
        if attrs.get('load_once') in ['true', True]:
            widgets = Container.get("AP_CustomLayoutWdg:widgets")
            if widgets == None:
                widgets = {}
                Container.put("AP_CustomLayoutWdg:widgets", widgets)
            else:
        ...
        if display_node is None:
            ...
            if not class_name:
                class_name = "tactic.ui.panel.AP_CustomLayoutWdg"
        ...
        try:
            ...
            config = WidgetConfigView('AP_CustomLayoutWdg', 'tmp', configs, state=my.state)
