...
class Snapshot(SObject):
    ...
    def set_latest(my, commit=True, update_versionless=True):
        ...
        if update_versionless:
            this_version = my.get_value("version")
            my.update_versionless("latest", this_version=this_version)
    ...
    def set_current(my, commit=True, update_versionless=True):
        ...
        if update_versionless:
            this_version = my.get_value("version")
            my.update_versionless("current", this_version=this_version)
    ...
    def update_versionless(my, snapshot_mode='current', sobject=None, checkin_type=None, naming=None, this_version=None):
        ...
        current_is_versionless = False
        try:
            current_is_versionless = naming.get_value("current_versionless")
        except:
            pass
        ...
        # Belal: if this is the first initial version in a snapshot, force it to be the current version
        if this_version == 1:
            my.set_value("is_current", True)

        if snapshot_mode == 'current' or not current_is_versionless:      # Belal: I hack here to create versionless only for 'current' instead of all
            ...
            for node in nodes:
                ...
                if 'v000' in patched_file_name:
                    patched_file_name = patched_file_name.replace('_v000', '')

                # reset the versionless file sobject to the new name
                file_object.set_value("file_name", patched_file_name)
                ...
                # make sure to fix the dir_naming, relative_dir_name and file_name and file_path for the versionless file of 'current_versionless'
                if 'v000' in new_path:
                    new_path = new_path.replace('_v000', '')
                    if '.versions' in new_path:
                        new_path = new_path.replace('/.versions', '')
                    new_lib_dir = new_lib_dir.replace('/.versions', '')
                    new_relative_dir = new_relative_dir.replace('/.versions', '')
