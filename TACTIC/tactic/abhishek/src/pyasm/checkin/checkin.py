...
class BaseCheckin(Command):
    ...
    def _execute(my):
        ...
        my.update_versionless("current", this_version=my.snapshot.get_value('version'))
        my.update_versionless("latest", this_version=my.snapshot.get_value('version'))
    ...
    def create_file_objects(my, file_paths):
        ...
        for i, file_path in enumerate(file_paths):
            if my.mode in ['local','inplace', 'move']:
                requires_file = False
            else:
                requires_file = True
    ...
    def update_versionless(my, snapshot_mode='current', this_version=None):
        ...
        return my.snapshot.update_versionless(snapshot_mode, sobject=my.sobject, checkin_type=my.checkin_type, naming=my.naming, this_version=this_version)
