                ...
                if mode == 'upload':
                    ...
                    if mode == 'move':
                        if os.path.isfile(file_path):
                            #belal commented this
                            #shutil.move(file_path, "%s/%s" % (handoff_dir, basename))
                            pass
                        else:
                            #belal commented this
                            #raise TacticApiException('[%s] is a directory. Use add_directory() instead! Cause Belal did not add support for "move" mode' %file_path)
                            pass
                    elif mode == 'copy':
                        if os.path.isfile(file_path):
                            shutil.copy(file_path, "%s/%s" % (handoff_dir, basename))
                        elif os.path.isdir(file_path):
                            shutil.copytree(file_path, "%s/%s" % (handoff_dir, basename), symlinks=True)
